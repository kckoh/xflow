...
import { Play, Loader2, ChevronUp, Filter, CheckSquare, Calendar } from "lucide-react";
...
export default function S3LogProcessEditor({
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="flex-1 flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4">
                        ...
                        <div className="border border-gray-200 rounded-lg p-3 space-y-3">
                            <h4 className="text-xs font-semibold text-gray-700 uppercase">Status Code Range</h4>

                            <div>
                                <label className="block text-xs text-gray-600 mb-1">Field Name</label>
                                <select
                                    value={filters.statusCodeField}
                                    onChange={(e) => handleFilterChange('statusCodeField', e.target.value)}
                                ...
                                >
                                    ...
                                    {sourceSchema.map((field, idx) => (
                                        <option key={idx} value={field.name}>{field.name}</option>
                                    ))}
                                </select>
                            </div>
                        ...
                        <div className="border border-gray-200 rounded-lg p-3 space-y-3">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => handleFilterChange('timestampField', e.target.value)}
                                    className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                                >
                                    <option value="">Select field...</option>
                                    {sourceSchema.map((field, idx) => (
            ...
            {/* Test Results */}
            <div className={`transition-all duration-500 ease-in-out border-t border-slate-100 bg-slate-50/30 overflow-hidden ${
                    ...
                    <div className="flex items-center justify-between mb-3 pb-1 border-b border-slate-200">
                        ...
                        {isTestSuccessful && testResult && (
                            <span className="text-xs font-bold text-green-600 flex items-center gap-1">
                                ✅ {testResult.filteredRecords} / {testResult.totalRecords} records
                            </span>
                        )}
                    </div>
                                    ...
                                    {testResult.afterRows.length > 0 ? (
                                        <table className="w-full text-xs box-border border-separate border-spacing-0">
                                            <thead className="bg-indigo-600 sticky top-0 z-10">
                                                <tr>
                                                    {Array.from(new Set(testResult.afterRows.flatMap(Object.keys))).map(key => (
                                            ...
                                            <tbody>
                                                {testResult.afterRows.slice(0, 5).map((row, i) => (
                                                    <tr key={i} className="hover:bg-indigo-50/30 transition-colors">
                                                        {Array.from(new Set(testResult.afterRows.flatMap(Object.keys))).map((key, j) => (

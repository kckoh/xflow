...
export default function SourceWizard() {
    ...
    const loadTablesOrCollections = async () => {
      ...
      try {
        if (selectedSource?.id === "postgres") {
          ...
          const response = await connectionApi.fetchKafkaTopics(
            config.connectionId
          );
          setTables(response.topics || []);
        }
      } catch (err) {
        console.error("Failed to load tables/collections/topics:", err);
        setTables([]);
      } finally {
  ...
  const handleTopicChange = async (topic) => {
    ...
    try {
      ...
      // Infer schema from Kafka topic
      const { schema, sample } = await connectionApi.inferKafkaSchema(
        config.connectionId,
        topic
      );
      ...
      setColumnMapping(
        schema.map((field) => ({
          source: { name: field.name, type: field.type },
          selected: true,
      ...
      console.log("Schema inferred successfully:", schema);
      console.log("Sample message:", sample);
    } catch (err) {
      console.error("Failed to infer Kafka schema:", err);
      alert(`Failed to infer schema: ${err.message}`);
      setConfig({ ...config, topic, columns: [] });
      setColumnMapping([]);
    } finally {
      setIsLoading(false);

...
def convert_duckdb_to_spark(sql: str) -> Tuple[str, List[str]]:
    """
    Convert DuckDB SQL to Spark SQL.

    Args:
        sql: DuckDB SQL string

    Returns:
        (converted SQL, list of converted functions)
    """
    converted_sql = sql
    conversions = []

    for duckdb_func, spark_func in DUCKDB_TO_SPARK.items():
        pattern = rf'\b{duckdb_func}\s*\('
        if re.search(pattern, converted_sql, re.IGNORECASE):
            ...
            conversions.append(f"{duckdb_func} -> {spark_func}")
...
def convert_spark_to_duckdb(sql: str) -> Tuple[str, List[str]]:
    ...
    match = re.search(lateral_view_pattern, converted_sql, re.IGNORECASE | re.VERBOSE | re.DOTALL)
    ...
    if match:
        ...
        for item in select_items:
            ...
            alias_col_match = re.match(rf'{alias}\.(\w+)(?:\s+as\s+(\w+))?', item_stripped, re.IGNORECASE)
    ...
    subquery_match = re.search(subquery_pattern, converted_sql, re.IGNORECASE | re.VERBOSE | re.DOTALL)
